...
LOG = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'attrfire_log.txt')
_lines = []
def log(s):
    _lines.append(s)
    print(s, flush=True)
...
def run_match(ue, pid, pawn):
    ...
    for method, typ in [(0,0),(1,0),(2,0),(3,0),(0,1),(1,1),(2,1),(3,1)]:
        ...
        try:
            saa('Atk', fp_atk, tmpl_base[0], method, 250.0, typ)
        except Exception as e:
            log(f"fire error {e}"); return None
        time.sleep(3)
        cur = rf32(set_inst + atk_off)
        log(f"  M={method} T={typ}: Atk={cur:.2f}")
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'attr_combo.txt'), 'w') as f:
